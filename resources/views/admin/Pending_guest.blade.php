...
@section('content')
    ...
    {{-- <script>
        ...
        window.viewAccessories = function(accessories) {
            ...
            const getMonthDiff = (start, end) => {
                ...
                if (isNaN(from) || isNaN(to)) return 'N/A';

                let months = (to.getFullYear() - from.getFullYear()) * 12 + (to.getMonth() - from.getMonth());

                // Only count the last month if the end day is greater than the start day
            ...
            const rows = accessories.map(item => {
                const name = item.accessory_head ? item.accessory_head.name : 'N/A';
                const price = item.pivot ? item.pivot.price : 'N/A';
                const totalAmount = item.pivot ? item.pivot.total_amount : 'N/A';
                const fromDate = item.pivot ? item.pivot.from_date : 'N/A';
                const toDate = item.pivot ? item.pivot.to_date : 'N/A';
                const monthCount = (fromDate !== 'N/A' && toDate !== 'N/A') ?
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            window.showProcessGuestModal = function(guestId, hasFeeWaiver) {
                ...
                console.log("Guest ID:", guestId, "Has Waiver:", hasFeeWaiver);
    ...
    <script>
         ...
         */
        const Api = {
            headers(json = false, csrf = false) {
                return {
                    'Accept': 'application/json',
                    ...(json ? { 'Content-Type': 'application/json' } : {}),
                    ...(csrf ? { 'X-CSRF-TOKEN': getCsrfToken() } : {}),
                    'token': localStorage.getItem('token'),
                    'auth-id': localStorage.getItem('auth-id')
                };
            },
            ...
            async post(url, data = {}, asJson = true, csrf = true) {
                console.log("🔄 POST:", url, data);
                try {
                    const options = {
                        method: "POST",
                        headers: this.headers(asJson, csrf),
        ...
        const UI = {
            alert(targetId, message, type = 'info', autoHide = true) {
                const container = document.getElementById(targetId);
                if (!container) {
                    console.warn(`⚠️ Target container #${targetId} not found.`);
                    return;
                }
                container.innerHTML = "";
            ...
            formErrors(containerId, errors) {
                ...
                if (typeof errors === "string") {
                    ...
                    for (const field in errors) {
                        box.innerHTML += `<div>${errors[field].join(', ')}</div>`;
                    }
                }
            },

            resetFormErrors(containerId) {
                const box = document.getElementById(containerId);
                if (box) {
                    box.textContent = "";
        ...
        const Guest = {
            async fetchPending() {
                ...
                if (!tbody) return console.error("❌ guestList not found.");

                tbody.innerHTML = `<tr><td colspan="17" class="text-center">Loading...</td></tr>`;
                try {
                    const res = await Api.get("{{ url('/api/admin/guests/pending') }}");
                    if (!res.success || !Array.isArray(res.data) || !res.data.length) {
                        tbody.innerHTML = `<tr><td colspan="17" class="text-center">No pending guests.</td></tr>`;
                        if (res.message) UI.alert("mainResponseMessage", res.message, "danger");
                        return;
                    }

                    tbody.innerHTML = res.data.map((g, i) => Guest.row(g, i)).join("");
                } catch {
                    tbody.innerHTML = `<tr><td colspan="17" class="text-danger text-center">Failed to load.</td></tr>`;
                }
            },

            row(g, i) {
                const accessoriesBtn = `<button class="btn btn-info btn-sm" onclick='Guest.viewAccessories(${JSON.stringify(g.accessories || [])})'>View</button>`;
                const feeWaiver = g.fee_waiver ? "Yes" : "No";
                const accountRemark = g.fee_exception?.account_remark || "N/A";
                const remarks = g.remarks || "N/A";
                const status = g.status || "N/A";
                const attachment = g.attachment_path
                    ? `<a href="/storage/${g.attachment_path}" target="_blank" class="btn btn-sm btn-secondary">View</a>`
                    : "N/A";

                return `
                    <tr id="guest-${g.id}">
                        <td>${i + 1}</td>
                        <td>${g.scholar_number || "N/A"}</td>
                        <td>${g.name || "-"}</td>
                        <td>${g.fathers_name || "N/A"}</td>
                        <td>${g.mothers_name || "N/A"}</td>
                        <td>${g.local_guardian_name || "N/A"}</td>
                        <td>${g.emergency_no || "N/A"}</td>
                        <td>${g.gender || "-"}</td>
                        <td>${g.room_preference || "N/A"}</td>
                        <td>${g.food_preference || "N/A"}</td>
                        <td>${feeWaiver}</td>
                        <td>${accountRemark}</td>
                        <td>${status}</td>
                        <td>${remarks}</td>
                        <td>${attachment}</td>
                        <td>${accessoriesBtn}</td>
                        <td>${Guest.actions(g)}</td>
                    </tr>
                `;
            },
            ...
            actions(g) {
                if (g.status === "pending") {
                    return g.fee_waiver
                        ? `<button class="btn btn-primary btn-sm" onclick="Guest.showProcessModal(${g.id}, true)">Process</button>
                           <button class="btn btn-danger btn-sm" onclick="Guest.reject(${g.id})">Reject</button>`
                        : `<button class="btn btn-success btn-sm" onclick="Guest.approve(${g.id})">Approve</button>
                           <button class="btn btn-danger btn-sm" onclick="Guest.reject(${g.id})">Reject</button>`;
                }
                if (g.status === "accountant_reject") {
                    return `<button class="btn btn-warning btn-sm" onclick="Guest.showReviewModal(${g.id})">Review</button>`;
                }
                return "";
            },

            async approve(id) {
                const res = await Api.post("{{ url('/api/admin/approve-guest') }}", { guest_id: id });
                res.success
                    ? (UI.alert("mainResponseMessage", res.message, "success"), Guest.fetchPending())
                    : UI.alert("mainResponseMessage", res.message, "danger");
            },

            async reject(id) {
                const remark = prompt("Enter rejection remark:");
                if (!remark) return;
                const res = await Api.post("{{ url('/api/admin/payment/reject') }}", { guest_id: id, admin_remarks: remark });
                res.success
                    ? (UI.alert("mainResponseMessage", res.message, "success"), Guest.fetchPending())
                    : UI.alert("mainResponseMessage", res.message, "danger");
            },

            async rejectWaiver(id) {
                const remark = prompt("Enter waiver rejection remark:");
                if (!remark) return;
                const res = await Api.post("{{ url('/api/admin/reject-waiver') }}", { guest_id: id, admin_remarks: remark });
                res.success
                    ? (UI.alert("mainResponseMessage", res.message, "success"), Guest.fetchPending())
                    : UI.alert("mainResponseMessage", res.message, "danger");
            },

            viewAccessories(list) {
                const container = document.getElementById("accessoryList");
                if (!container) return;
                if (!list.length) {
                    container.innerHTML = "<p>No accessories.</p>";
                } else {
                    container.innerHTML = `
                        <table class="table table-striped">
                          <thead><tr><th>Name</th><th>Price</th><th>From</th><th>To</th><th>Months</th><th>Total</th></tr></thead>
                          <tbody>
                            ${list.map(a => `
                              <tr>
                                <td>${a.accessory_head?.name || "N/A"}</td>
                                <td>${a.pivot?.price || "N/A"}</td>
                                <td>${a.pivot?.from_date || "N/A"}</td>
                                <td>${a.pivot?.to_date || "N/A"}</td>
                                <td>${Guest.monthDiff(a.pivot?.from_date, a.pivot?.to_date)}</td>
                                <td>${a.pivot?.total_amount || "N/A"}</td>
                              </tr>`).join("")}
                          </tbody>
                        </table>`;
                }
            ...
            showProcessModal(id, hasWaiver) {
                const c = document.getElementById("processGuestButtons");
                if (!c) return;
                c.innerHTML = hasWaiver
                    ? `<button class="btn btn-primary" onclick="Guest.approveWaiver(${id})">Approve Waiver</button>
                       <button class="btn btn-danger" onclick="Guest.rejectWaiver(${id})">Reject Waiver</button>`
                    : `<button class="btn btn-success" onclick="Guest.approve(${id})">Approve</button>
                       <button class="btn btn-danger" onclick="Guest.reject(${id})">Reject</button>`;
                confirmProcessModal.show();
            },

            approveWaiver(id) {
                confirmProcessModal.hide();
                Guest.showAdjustModal(id);
            },

            async showAdjustModal(id) {
                console.log("Opening adjust modal for", id);
                // Reset form fields here...
                editAmountModal.show();
                // Fetch details and fill form...
            },

            async showReviewModal(id) {
                console.log("Opening review modal for", id);
                reviewRejectModal.show();
                // Fetch details and fill form...
            }
        };
         ...
         * =========================
         */
        let editAmountModal, reviewRejectModal, confirmProcessModal;
        document.addEventListener("DOMContentLoaded", () => {
            editAmountModal = new bootstrap.Modal("#editAmountModal", { backdrop: 'static', keyboard: false });
            reviewRejectModal = new bootstrap.Modal("#reviewRejectModal", { backdrop: 'static', keyboard: false });
            confirmProcessModal = new bootstrap.Modal("#confirmProcessModal", { backdrop: 'static', keyboard: false });
