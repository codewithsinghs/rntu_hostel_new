    ...
    public function guestLogin(Request $request)
    {
        try {
            $credentials = $request->validate([
                ...
                // 'mobile' => ['required', 'regex:/^[6-9][0-9]{9}$/'], // Validates Indian mobile numbers
                'mobile' => ['required'],
            ]);

            if ($guest_details = Guest::where('email', $credentials['email'])->where('mobile', $credentials['mobile'])->first()) {
